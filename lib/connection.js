...
async function startBot() {
    ...
    conn.ev.on('connection.update', (update) => {
        ...
        if (connection === 'close') {
            ...
            else console.log('⚠️ Logged out. Delete auth_info folder and re-run.');
    ...
    conn.ev.on('messages.upsert', async (m) => {
        ...
        const ownerNumber = config.OWNER_NUMBER || 'owner_number';
        ...
        if (body.startsWith(prefix)) {
            ...
            // Alive command
            if (cmd === 'alive') {
                let text = `👑 Hello ${ownerName} (${ownerNumber}) 👑\nYour bot is online 🥰`;
